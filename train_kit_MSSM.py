...
from models.mask_transformer.transformer_mamba3D import Maskmamba
from models.mask_transformer.transformer_trainer3D import MaskMambaTrainer
from models.vq.model import RVQVAE

from options.train_kit.option_kit_MSSM_train import TrainT2MOptions
...
def plot_t2m(data, save_dir, captions, m_lengths):
    ...
    ##data (3,196,263) [max52.87,min-65.90]   m_lengths=[196,104,184]
    # print(ep_curves.shape)
    for i, (caption, joint_data) in enumerate(zip(captions, data)):
        joint_data = joint_data[:m_lengths[i]]  ### joint =196,263
        joint = recover_from_ric(torch.from_numpy(joint_data).float(), opt.joints_num).numpy()
        save_path = pjoin(save_dir, '%02d.mp4'%i) ###joint[196,22,3]
        print(joint.shape)
...
if __name__ == '__main__':
    ...
    t2m_transformer = Maskmamba(code_dim=vq_opt.code_dim,
                                      cond_mode='text',
    ...
    trainer = MaskMambaTrainer(opt, t2m_transformer, vq_model)
